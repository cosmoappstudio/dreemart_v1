...
export default function MainApp() {
  ...
  const handleGenerate = async () => {
    ...
    if (tier === 'FREE' && credits < needed) {
      ...
      metaTrackCustom('paywall_opened', { reason: 'insufficient_credits' });
      return;
    }
    ...
    try {
      ...
      for (let i = 0; i < selectedArtists.length; i++) {
        ...
        if (!res.ok) {
          if (res.status === 402) {
            ...
            metaTrackCustom('paywall_opened', { reason: 'api_402' });
            setPaywallView('credits');
            setShowPaywall(true);
          }
      ...
      metaTrackCustom('dream_generated', { artist_count: results.length });
  ...
  const handleDownload = async (imageUrl: string, filename = 'dreemart-dream.png') => {
    ...
    metaTrackCustom('download_clicked', { from: 'dashboard' });
    try {
  ...
  const handleShare = async (imageUrl: string, title?: string) => {
    ...
    metaTrackCustom('share_clicked', { from: 'dashboard' });
      ...
      <div className="relative z-10 w-full md:max-w-5xl md:max-h-[90vh] md:rounded-2xl md:shadow-2xl md:border md:border-white/10 flex flex-col md:flex-row bg-[#0B0D17] overflow-hidden flex-1 md:flex-initial">
        ...
        <div className="flex-1 min-w-0 min-h-0 flex flex-col p-6 pb-28 md:pb-6 overflow-y-auto bg-[#0B0D17]/95 md:bg-[#1a1c2e]">
          ...
          <div className="flex flex-col gap-3">
            ...
            <button onClick={() => { trackEvent('tab_switch', { tab: 'gallery' }); metaTrackCustom('tab_switch', { tab: 'gallery' }); setShowSuccessView(false); setActiveTab('gallery'); }} className="w-full py-3 rounded-xl bg-white/5 border border-white/10 text-gray-300 hover:bg-white/10 flex items-center justify-center gap-2 min-h-[48px] touch-manipulation"><LayoutGrid className="w-4 h-4" />{t('goToGallery')}</button>
          </div>
        </div>
      </div>
    </div>
      ...
      <main className="flex-1 px-4 sm:px-6 flex flex-col gap-8 pb-24 md:pb-6 overflow-y-auto overflow-x-hidden max-w-3xl mx-auto w-full pt-4 sm:pt-6 md:pt-12">
        ...
        {tier === 'FREE' && (
          <section className="md:hidden p-4 rounded-xl bg-gradient-to-r from-gold-500/10 to-amber-500/10 border border-gold-500/20">
            <button onClick={() => { trackEvent('paywall_opened', { source: 'home_section' });
            metaTrackCustom('paywall_opened', { source: 'home_section' }); setPaywallView('credits'); setShowPaywall(true); }} className="w-full py-3 rounded-xl font-bold text-sm bg-gradient-to-r from-gold-500/20 to-amber-500/20 border border-gold-500/30 text-gold-300 hover:from-gold-500/30 hover:to-amber-500/30 transition-colors flex items-center justify-center gap-2 min-h-[48px] touch-manipulation">
      ...
      <main className="flex-1 px-4 pb-24 overflow-y-auto">
        {dreams.length === 0 ? (
          <div className="flex flex-col items-center justify-center text-gray-500 py-20">
            ...
            <button onClick={() => { trackEvent('tab_switch', { tab: 'home' }); metaTrackCustom('tab_switch', { tab: 'home' }); setActiveTab('home'); }} className="mt-6 px-6 py-2 bg-white/10 hover:bg-white/20 rounded-full text-sm font-bold text-white">{t('createNow')}</button>
          </div>
        ) : (
      ...
      <main className="flex-1 px-6 flex flex-col gap-6 pb-24 overflow-y-auto max-w-3xl mx-auto w-full pt-6">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="p-4 rounded-xl bg-white/5 border border-white/10">
            ...
            {tier === 'FREE' && <button onClick={() => { trackEvent('paywall_opened', { source: 'profile_stats' });
            metaTrackCustom('paywall_opened', { source: 'profile_stats' }); setPaywallView('credits'); setShowPaywall(true); }} className="text-xs text-gold-400 hover:text-gold-300 font-bold mt-1 text-left min-h-[44px] touch-manipulation flex items-center">{t('loadCredits')}</button>}
          </div>
          <div className="p-4 rounded-xl bg-white/5 border border-white/10">
  ...
  const Sidebar = () => (
    <div className="hidden md:flex flex-col w-64 flex-shrink-0 border-r border-white/10 bg-[#0B0D17]/50 p-4 gap-2 h-screen sticky top-0">
      ...
      <button onClick={() => { trackEvent('tab_switch', { tab: 'home' }); metaTrackCustom('tab_switch', { tab: 'home' }); setActiveTab('home'); }} className={`flex items-center gap-3 px-4 py-3 rounded-xl text-left ${activeTab === 'home' ? 'bg-white/10 text-white font-bold' : 'text-gray-400 hover:bg-white/5'}`}><Home className="w-5 h-5" />{t('tabHome')}</button>
      <button onClick={() => { trackEvent('tab_switch', { tab: 'gallery' }); metaTrackCustom('tab_switch', { tab: 'gallery' }); setActiveTab('gallery'); }} className={`flex items-center gap-3 px-4 py-3 rounded-xl text-left ${activeTab === 'gallery' ? 'bg-white/10 text-white font-bold' : 'text-gray-400 hover:bg-white/5'}`}><LayoutGrid className="w-5 h-5" />{t('tabGallery')}</button>
      <button onClick={() => { trackEvent('tab_switch', { tab: 'profile' }); metaTrackCustom('tab_switch', { tab: 'profile' }); setActiveTab('profile'); }} className={`flex items-center gap-3 px-4 py-3 rounded-xl text-left ${activeTab === 'profile' ? 'bg-white/10 text-white font-bold' : 'text-gray-400 hover:bg-white/5'}`}><User className="w-5 h-5" />{t('tabProfile')}</button>
      <div className="mt-auto pt-4 border-t border-white/5 space-y-1">
        ...
        <button onClick={() => { trackEvent('paywall_opened', { source: 'sidebar' });
        metaTrackCustom('paywall_opened', { source: 'sidebar' }); setPaywallView('credits'); setShowPaywall(true); }} className="w-full mt-3 flex items-center justify-between bg-gradient-to-r from-purple-900/40 to-blue-900/40 px-4 py-3 rounded-xl border border-white/5">
        ...
        {!showSuccessView && (
          <div className="md:hidden fixed bottom-0 left-0 right-0 z-40 w-full bg-[#0B0D17]/90 backdrop-blur-lg border-t border-white/10 pb-safe">
            <div className="flex justify-around items-center h-14 min-h-[56px]">
              <button onClick={() => { trackEvent('tab_switch', { tab: 'home' }); metaTrackCustom('tab_switch', { tab: 'home' }); setActiveTab('home'); }} className={`flex flex-col items-center gap-0.5 w-full h-full justify-center min-h-[44px] touch-manipulation ${activeTab === 'home' ? 'text-gold-400' : 'text-gray-500'}`}><Home className="w-6 h-6" /><span className="text-[10px] font-bold">{t('tabHome')}</span></button>
              <button onClick={() => { trackEvent('tab_switch', { tab: 'gallery' }); metaTrackCustom('tab_switch', { tab: 'gallery' }); setActiveTab('gallery'); }} className={`flex flex-col items-center gap-0.5 w-full h-full justify-center min-h-[44px] touch-manipulation ${activeTab === 'gallery' ? 'text-gold-400' : 'text-gray-500'}`}><LayoutGrid className="w-6 h-6" /><span className="text-[10px] font-bold">{t('tabGallery')}</span></button>
              <button onClick={() => { trackEvent('tab_switch', { tab: 'profile' }); metaTrackCustom('tab_switch', { tab: 'profile' }); setActiveTab('profile'); }} className={`flex flex-col items-center gap-0.5 w-full h-full justify-center min-h-[44px] touch-manipulation ${activeTab === 'profile' ? 'text-gold-400' : 'text-gray-500'}`}><User className="w-6 h-6" /><span className="text-[10px] font-bold">{t('tabProfile')}</span></button>
            </div>
          </div>
        )}
